...
class User extends Eloquent {
    ...
    protected $fillable = array('user_name', 'user_password', 'user_full_name', 'user_email', 'user_employee_id', 'user_status', 'user_is_admin', 'user_group','user_last_login','user_last_ip','user_create_id','user_create_name','user_edit_id','user_edit_name','user_created','user_updated');
    ...
    public static function searchByCondition($data = array(), $limit = 0, $offset = 0, &$size)
    {
        ...
        try {
            ...
            if (isset($data['user_employee_id']) && $data['user_employee_id'] != 0) {
                $query->where('user_employee_id', $data['user_employee_id']);
            }
