...
//        }
    ...
    buildBoxProduct :function(key,type){
        ...
        if(key == -1 || checkBox == 0 || type ==1){
            for( var i in productForBox){
                ...
                if(key == -2){
                    html += '<td><div class="checkbox"><label><input class="uniform" type="radio" name="product_is_default" id="product_is_default_'+index+'" checked></label></div>';
                }else{
                    html += '<td><div class="checkbox"><label><input class="uniform" type="radio" name="product_is_default" id="product_is_default_'+index+'"></label></div>';
                }

                for( var j in storeAdd){
                    html +='<td><div>';
                    html += '<input maxlength="10" autocomplete="off" onkeyup="javascript:campaign.validateQuanlity('+storeAdd[j].store_supplier_id+','+index+');"  type="text" id="sys_store_quanlity_'+storeAdd[j].store_supplier_id+'_'+index+'" name="sys_store_quanlity_'+storeAdd[j].store_supplier_id+'_'+index+'"  placeholder="" class="form-control" />';
                    html +=' </div></td>';
                }
                html += '</tr>';
            }
    ...
    saveProduct :function(key){
        ...
        for( var i in productForBox){
            ...
            store_product_temp[product_id] = store_supplier_campaign;
            ...
            if(id > 0 && checkQuanlity > 0 && key != -1){
                if(arrProductEdit[campaign_box_name] == undefined ||!this.inArray(product_id,arrProductEdit[campaign_box_name])){
                    var itemProduct = {
                        ...
                        store_supplier_campaign : store_product_temp[product_id],
            ...
            for(var p in dataProduct){
                if(arrProductEdit[nameBox] != undefined && typeof p != 'undefined' && this.inArray(parseInt(p),arrProductEdit[nameBox])){
                    var product_id = parseInt(p),
                        ...
                        //store_supplier_campaign = BOX[key].campaign_products[product_id].store_supplier_campaign,
                        product_price = arrProductPrice[product_id],
        ...
        if(checkQuanlity > 0){
            if(product_default > 0){
                if(!isNaN(quanlity_checked) && quanlity_checked > 0){
                    ...
                    alert('Hãy nhập số lượng cho sản phẩm đại diện cho Box');
                    return false;
                }
            }
            if(id > 0 && key != -1 && campaign_box_id > 0){
    ...
    buildHtmlTitle :function(key,campaign_box_name){
        var html = '<span id="title_'+key+'"><a href="javascript:campaign.showEditBox('+key+',0);">';
...
//            $('#sys_next').attr('disabled', true);
        ...
        var percent = $('#sys_campaign_percent').val(),//mức giảm
            ...
            campaign_category= $('#sys_campaign_category').val(),
        ...
        if($.isEmptyObject(BOX) && sys_id_campaign == '' && $.isEmptyObject(arrProductChecked)){
            ...
            if($.isEmptyObject(arrProductChecked)){
                mess += "Phải chọn sản phẩm mới qua được bước tiếp theo </br>";
            }
        }
        ...
        if(!this.checkEmpty(product_default) || product_default == 0){
            mess +='Hãy chọn 1 sản phẩm làm đại diện cho campaign </br>';
        }

        if(this.checkEmpty(campaign_expired_coupon)){
            if(this.checkNumber(campaign_expired_coupon,1) == 1){
                mess += 'Thời gian giữ hàng không phải là số </br>';
            }else if(this.checkNumber(campaign_expired_coupon,1) == 0){
                mess += 'Thời gian giữ hàng không được nhỏ hơn 0 </br>';
            }
        }

        if(campaign_category == 0){
            mess += 'Hãy chọn 1 danh mục cho campaign';
        }else{
            if(this.inArray(campaign_category,cateParent)){
                mess += 'Không chọn danh mục cha,hãy chọn danh mục con';
            }
        }
