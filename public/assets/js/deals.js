...
var deals = {
    ...
    addProduct: function() {
        ...
        $.ajax({
            dataType: 'json',
            type: 'post',
            url: WEB_ROOT + 'shop/campaign/ajaxUploadProduct',
            data: {
    ...
    editProductBox: function(id) {
        ...
        $('#sys_label_edit_'+id).attr('onclick', 'deals.saveProductBox('+id+', 2)');
    },

    editProductBox2: function(id) {
        $('.show_campaign_number_init_'+ id).hide();
        $('.sys_campaign_number_init_'+id).show();
        $('#sys_label_edit_'+id).html('Lưu lại');
        $('#sys_label_edit_'+id).attr('onclick', 'deals.saveProductBox('+id+', 1)');
    },

    saveProductBox: function(id, type) {
        ...
        if(type==1) {
            $('#sys_label_edit_'+id).attr('onclick', 'deals.editProductBox2('+id+')');
        }else{
    ...
    showListProduct: function() {
        ...
        $.ajax({
            dataType: 'json',
            type: 'post',
            url: WEB_ROOT + 'shop/campaign/getListDataSupplier',
            data: {
...
//                }
            ...
            if(statusExitst == 0) {
                ...
                html += '</td>';
                html += '<td><a id="sys_label_edit_'+deals.cfg.arrProductLoad[i].products_id+'" href="javascript:void(0);" onclick="deals.saveProductBox('+ deals.cfg.arrProductLoad[i].products_id +', 2);">Lưu lại</a> &nbsp;|&nbsp; <a href="javascript:void(0);" onclick="deals.deleteProductBox('+ deals.cfg.arrProductLoad[i].products_id +');">Xóa</a></td></tr>';
                $('#dataTable').find('tbody').append(html);
                deals.cfg.countProduct++;
            }
            j++;
