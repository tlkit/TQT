   ...
   initilize :function(){
    ...
    showPromotionType : function(key){
        ...
        if(key == 1){
            ...
            $('#sys_campaign_deal_input').val('1');
            $('#sys_campaign1').show();
            $('#sys_campaign2').hide();
            $("#sys_label_percent").addClass("textleft");
            $("#sys_short_descrip").show();
            $("#sys_content").show();
            $("#sys_title_box").text('Nhập box');             
        }else{
            $('#sys_campaign_deal_input').val('2');
            $('#sys_campaign2').show();
            $('#sys_campaign1').hide();
            $("#sys_label_percent").removeClass("textleft");      
            $("#sys_short_descrip").hide();
            $("#sys_content").hide();
            $("#sys_title_box").text('Thêm sản phẩm');
        }
        $("#sys_campaign_deal").val(key);
    ...
    buildBoxProduct :function(key,type){
        ...
        if(key == -1 || checkBox == 0 || type ==1){
            for( var i in productForBox){
                ...
                html += '<tr>';
                html += '<td><p class="word2">' + arrProductKey[index]+'</p></td>';
